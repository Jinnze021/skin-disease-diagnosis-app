...
import com.project.demo.entity.HospitalInformation;
import com.project.demo.service.HospitalInformationService;
...
/**
 * 医院信息：(HospitalInformation)表控制层
 *
 */
@RestController
@RequestMapping("/hospital_information")
public class HospitalInformationController extends BaseController<HospitalInformation, HospitalInformationService> {

    /**
     * 医院信息对象
     */
    @Autowired
    public HospitalInformationController(HospitalInformationService service) {
        setService(service);
    }
        ...
        if (paramMap.get("user_id")==null||paramMap.get("user_id").equals("")){
            ...
            try {
                ...
                e.printStackTrace();
						            ...
						            //查询该用户常点击
            String hitsSource = "SELECT COUNT( hits_id ) AS hits_count, source_id FROM hits WHERE source_table = 'hospital_information' AND user_id = "+paramMap.get("user_id")+" GROUP BY source_id";
            ...
            if (hitsSourceList==null || hitsSourceList.size()<=0){
								                hitsSource = "SELECT COUNT( hits_id ) AS hits_count, source_id FROM hits WHERE source_table = 'hospital_information' GROUP BY source_id";
												                hitsSourceList = service.selectMapBaseList(hitsSource);
				            }
						String table_name = "hospital_information";
			String table_id = "hospital_information_id";
			String type_name = "type_of_hospital";
                ...
                if (typeList==null || typeList.size()<=0){
                    ...
                    if (resultList==null || resultList.size()<=0){
                        ...
                        for (int i = resultList.size() - 1; i > 0; i--) {
                            ...
                            for (int j = 0; j < i; j++) {
								                                HospitalInformation leftMap = (HospitalInformation) resultList.get(j);
                                HospitalInformation rightMap = (HospitalInformation) resultList.get(j + 1);
                                if (leftMap.getHits()<rightMap.getHits()){
                                    HospitalInformation temp = rightMap;
                                    rightMap = leftMap;
                                    leftMap = temp;
                                    swapped = true;
                                }
